<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.4" tiledversion="2020.10.30" name="museum_gaffa" tilewidth="32" tileheight="32" tilecount="81" columns="9">
 <image source="museum_gaffa.png" width="288" height="288"/>
 <tile id="0">
  <animation>
   <frame tileid="0" duration="1000"/>
   <frame tileid="61" duration="100"/>
  </animation>
 </tile>
 <tile id="1">
  <animation>
   <frame tileid="1" duration="1000"/>
   <frame tileid="62" duration="100"/>
  </animation>
 </tile>
 <tile id="2">
  <animation>
   <frame tileid="2" duration="1000"/>
   <frame tileid="71" duration="100"/>
  </animation>
 </tile>
 <tile id="6">
  <animation>
   <frame tileid="6" duration="200"/>
   <frame tileid="7" duration="200"/>
   <frame tileid="8" duration="200"/>
  </animation>
 </tile>
</tileset>
